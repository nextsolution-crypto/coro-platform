...
let RulesEngineService = class RulesEngineService {
    analyzeConfiguration(config) {
        ...
        result.rolesRecommandes = [...new Set(result.rolesRecommandes.filter(r => !result.rolesActives.includes(r)))];
        result.proceduresActives = [...new Set(result.proceduresActives)];
        result.sectionsDocument = [...new Set(result.sectionsDocument)];
        return result;
    }
    applyBaseRules(config, result) {
    ...
    applyGicleurRules(config, result) {
        if (config.gicleurs) {
    ...
    applyExtincteurRules(config, result) {
    ...
    applyMechanicalRules(config, result) {
    ...
    applyHazardRules(config, result) {
    ...
    applyIndustrialRules(config, result) {
    ...
    applyOccupancyRules(config, result) {
    ...
    applyCommunicationRules(config, result) {
    ...
    applyCertificationRules(config, result) {
        if (config.certBOMA) {
    ...
    applySectionRules(config, result) {
        result.sectionsDocument.push('PAGE_COUVERTURE', 'TABLE_MATIERES', 'INTRODUCTION', 'OBJET_PORTEE', 'RESPONSABILITE_CONTENU', 'FORMATION_FREQUENCE', 'EXERCICE_EVACUATION', 'HISTORIQUE_MISES_JOUR', 'DEFINITIONS_TERMES', 'LISTE_TELEPHONIQUE', 'NUMEROS_URGENCE', 'RESSOURCES_INTERNES', 'RESSOURCES_EXTERNES', 'ROLES_RESPONSABILITES', 'ORGANIGRAMME_URGENCE', 'PROCEDURES_EQUIPE_URGENCE', 'DECOUVERTE_FEU', 'PLANS_TECHNIQUES', 'DESCRIPTION_GENERALE', 'MECANIQUE_BATIMENT', 'ALARME_INCENDIE', 'EXTINCTEUR_PORTATIF', 'EQUIPEMENTS_PREMIERS_SOINS', 'PHOTOS_EQUIPEMENTS', 'REGISTRE_EVACUATION_SECTEURS', 'REGISTRE_FORMATION', 'RAPPORT_EVACUATION', 'INSPECTION_EQUIPEMENTS');
        if (config.gicleurs)
            result.sectionsDocument.push('SYSTEME_GICLEURS');
        if (config.matieresDangereuses)
            result.sectionsDocument.push('MATIERES_DANGEREUSES');
        if (config.detecteurCO || config.detecteurGazNaturel || config.detecteurAmmoniac)
            result.sectionsDocument.push('DETECTEURS_GAZ');
        if (config.ammoniac)
            result.sectionsDocument.push('PROCEDURES_AMMONIAC');
        if (config.espaceClos)
            result.sectionsDocument.push('ESPACE_CLOS_CADENASSAGE');
        if (config.travailChaud)
            result.sectionsDocument.push('PERMIS_TRAVAIL_CHAUD');
        if (config.chariotsElevateurs || config.palettiers)
            result.sectionsDocument.push('ENTREPOSAGE_MANUTENTION');
        if (config.procesDangereux)
            result.sectionsDocument.push('PROCEDES_DANGEREUX');
        if (config.messagesAutomatises || config.systemePhonic)
            result.sectionsDocument.push('MESSAGES_PHONIQUES');
        if (config.batteriesLithium)
            result.sectionsDocument.push('FEU_BATTERIE_LITHIUM');
    }
    applyValidations(config, result) {
