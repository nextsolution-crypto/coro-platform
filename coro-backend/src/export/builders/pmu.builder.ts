...
export class PmuBuilder extends BaseDocumentBuilder {
  ...
  async buildModuleSegments(
    ...
    // ── Tous les autres modules : flush du chunk HTML courant ──
    if (currentHtmlChunk.value) {
      pdfSegments.push({
        type: 'html',
        content: currentHtmlChunk.value,
    ...
    const sepHtml = this.buildSeparatorHtml(
      moduleNum,
      generateSeparatorPage,
    );
    ...
    if (moduleNum === 3) {
      ...
      const mergedSections3 = this.mergeModule3SavedData(
        mod?.sections || [],
        this.content.module3,
      );

      const { html31, html32, has32 } = renderModule3(
        mergedSections3,
        this.lang,
        this.sequentialNumber,
      );
      ...
      const s31Title =
        mergedSections3.find((s: any) => s.id === '3.1')?.title ||
        (this.isFr ? 'ORGANIGRAMME' : 'ORGANIZATIONAL CHART');

      this.subsectionTitlesById[
        `${this.sequentialNumber}:3.1`
      ] = `${this.sequentialNumber}.1 — ${s31Title}`;
      ...
      if (has32 && this.isIndustriel) {
        const s32Title =
          mergedSections3.find((s: any) => s.id === '3.2')?.title ||
          (this.isFr ? 'LISTE DES MEMBRES' : 'MEMBER LIST');

        this.subsectionTitlesById[
          `${this.sequentialNumber}:3.2`
        ] = `${this.sequentialNumber}.2 — ${s32Title}`;
    ...
    // ── Module 7 ──
    if (moduleNum === 7) {
      const module7Data = await this.prisma.module7Data.findUnique({
