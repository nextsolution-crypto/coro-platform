...
export class PsiBuilder extends PmuBuilder {
  ...
  async buildModuleSegments(
    ...
    if (moduleNum === 6) {
      return super.buildModuleSegments(
        moduleNum,
        pdfSegments,
        currentHtmlChunk,
        currentHtmlSeqNumber,
      );
    }
    ...
    if (currentHtmlChunk.value) {
      pdfSegments.push({
        type: 'html',
        content: currentHtmlChunk.value,
        sequentialNumber: currentHtmlSeqNumber.value,
      });
    ...
    if (moduleNum === 1) {
      ...
      const sections1 = allSections.filter(
        (s: any) => !PSI_DISABLED_SECTIONS_M1.includes(s.id),
      );

      const psiSections = sections1.map((s: any) =>
        this.adaptSectionForPsi(s),
      );

      psiSections.forEach((s: any) => {
        this.subsectionTitlesById[
          `${this.sequentialNumber}:${s.id}`
        ] = `${s.id} — ${s.title}`;
      });
    ...
    if (moduleNum === 2) {
      ...
      const mergedSections = this.mergeModule2SavedData(
        mod?.sections || [],
        this.content.module2,
      );

      mergedSections.forEach((s: any, i: number) => {
        this.subsectionTitlesById[
          `${this.sequentialNumber}:${s.id}`
        ] = `${this.sequentialNumber}.${i + 1} — ${s.title}`;
      });

      for (let i = 0; i < mergedSections.length; i++) {
        pdfSegments.push({
          type: 'html',
          content: `<div>${renderModule2Section(
            mergedSections[i],
            i,
            this.lang,
    ...
    //
    // IMPORTANT :
    // aucune whitelist P001/P002.
    // documentTypes + activationRule + sélection manuelle + activation projet
    // sont résolus centralement dans BaseDocumentBuilder.
    if (moduleNum === 4) {
      const procedures = await this.getModule4Procedures();

      const buildingAddress =
        `${this.project.building.address}, ` +
        `${this.project.building.city}, ` +
        `${this.project.building.province}`;
      ...
      procedures.forEach((proc: any) => {
        ...
        this.subsectionTitlesById[
          `${this.sequentialNumber}:${proc.id}`
        ] = `${proc.code} — ${title}`;

        pdfSegments.push({
          type: 'html',
          content: `<div>${renderProcedure(
            proc,
            this.lang,
            buildingAddress,
            this.content.config,
    ...
    if (moduleNum === 7) {
      const module7Data =
        await this.prisma.module7Data.findUnique({
          where: { projectId: this.project.id },
        });

      const sections7 = renderModule7(
        module7Data,
        this.content.config,
        this.lang,
        this.sequentialNumber,
      ...
      sections7.forEach((section, idx) => {
        this.subsectionTitlesById[
          `${this.sequentialNumber}:${section.id}`
        ] = `${this.sequentialNumber}.${idx + 1} — ${section.title}`;
    ...
    if (moduleNum === 8) {
      ...
      // Choix métier PSI : section 8.11 exclue.
      const sections8 = allSections8.filter(
        (s: any) => s.id !== '8.11',
      );
  ...
  private adaptSectionForPsi(section: any): any {
    ...
    if (adapted.content) {
      adapted.content =
        this.replacePmuWithPsi(adapted.content);
    }

    if (adapted.subsections) {
      adapted.subsections =
        adapted.subsections.map(
          (sub: any) => this.adaptSectionForPsi(sub),
        );
    }

    if (adapted.paragraphs) {
      adapted.paragraphs =
        adapted.paragraphs.map((p: any) => ({
          ...p,
          content: p.content
            ? this.replacePmuWithPsi(p.content)
            : p.content,
          text: p.text
            ? this.replacePmuWithPsi(p.text)
            : p.text,
        }));
    }
    ...
    if (adapted.bullets) {
      adapted.bullets =
        adapted.bullets.map((b: any) =>
          typeof b === 'string'
            ? this.replacePmuWithPsi(b)
            : {
                ...b,
                text: b.text
                  ? this.replacePmuWithPsi(b.text)
                  : b.text,
              },
        );
    }
  ...
  private replacePmuWithPsi(text: string): string {
    ...
    return text
      ...
      .replace(
        /code national de prévention des incendies – Canada 2010/gi,
