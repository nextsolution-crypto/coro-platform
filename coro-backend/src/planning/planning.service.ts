  ...
  private async snapshot(query: PlanningWindowDto, actor: Actor, withCapacity: boolean) {
    ...
    const [bookings, schedules, absences, activities, capacityRows] = await Promise.all([
      this.prisma.booking.findMany({ where: { organizationId: actor.organizationId,
        ...
        activityType: true, projectId: true, project: { select: { name: true, clientId: true, buildingId: true,
          building: { select: { timeZone: true, timeZoneVerified: true } } } },
      ...
      this.prisma.projectActivity.findMany({ where: { organizationId: actor.organizationId,
        ...
        label: true, customLabel: true, assigneeEmail: true, sourceMandate: true, clientBookable: true,
        projectId: true, project: { select: { clientId: true, buildingId: true,
          building: { select: { timeZone: true, timeZoneVerified: true } } } },
    ...
    for (const booking of bookings) {
      ...
      if (relevant) {
        ...
        events.push({ id: `booking:${booking.id}`, source: 'BOOKING', startUtc: interval.startUtc,
          ...
          label: detailed ? booking.status === 'DEMANDEE' ? 'Demande client — non confirmée' : booking.activityType : 'Occupé',
