// ============================================================
// CORO — Module 2 : Liste téléphonique
// Patron identique au Module 1 : generateModule2(ctx)
// retourne { fr: {...}, en: {...} }
// ============================================================
...
// ============================================================
// TYPES SPÉCIFIQUES AU MODULE 2
// ============================================================

export interface PhoneEntry {
  role: string;       // affiché dans la colonne Rôle/Équipement
  name: string;       // colonne Nom (vide par défaut = à remplir)
  phone: string;      // colonne Téléphone
  isBold?: boolean;   // true = ligne en gras (ex: Coordonnateur)
  isFixed?: boolean;  // true = entrée pré-remplie système (non supprimable)
}

export interface ExternalResource {
  role: string;       // Rôle / organisme
  phone: string;      // Numéro(s) de téléphone
  url?: string;       // Lien hypertexte optionnel
  isBold?: boolean;   // Ligne surlignée (ex: GardaWorld)
  isFixed?: boolean;  // Entrée système non supprimable
}

export interface Module2SectionData {
  type: 'phone_table' | 'external_table';
  entries: PhoneEntry[] | ExternalResource[];
  availableRoles?: string[];  // Pour les dropdowns 2.2 et 2.3
}

// ============================================================
// LISTES DE RÔLES POUR LES DROPDOWNS
// ============================================================

export const ROLES_INTERNES_BUREAU_FR = [
...
export const EQUIPEMENTS_BASE_FR = [
...
export const EQUIPEMENTS_BASE_EN = [
...
export const EQUIPEMENTS_CONDITIONNELS_FR: Record<string, string[]> = {
  has_sprinklers: [
...
export const EQUIPEMENTS_CONDITIONNELS_EN: Record<string, string[]> = {
  has_sprinklers: [
...
// ============================================================
// TOUTES LES OPTIONS DROPDOWN ÉQUIPEMENTS (pour l'UI)
// ============================================================
...
export const ALL_EQUIPEMENTS_EN = [
  ...EQUIPEMENTS_BASE_EN,
  'Sprinkler system maintenance',
  'Generator maintenance',
  'Generator fuel service',
...
// ============================================================
// RESSOURCES EXTERNES PAR PROVINCE
// ============================================================

const RESSOURCES_COMMUNES_FR: ExternalResource[] = [
...
const RESSOURCES_QC_EN: ExternalResource[] = [
...
const RESSOURCES_ON_FR: ExternalResource[] = [
...
const RESSOURCES_ON_EN: ExternalResource[] = [
...
const RESSOURCES_AB_FR: ExternalResource[] = [
...
const RESSOURCES_AB_EN: ExternalResource[] = [
  { role: 'Alberta Poison Centre', phone: '1 (800) 332-1414' },
...
function getExternalResources(province: string, lang: 'fr' | 'en'): ExternalResource[] {
  const p = province?.toLowerCase();
  if (lang === 'fr') {
    if (p === 'qc' || p === 'québec' || p === 'quebec') return RESSOURCES_QC_FR;
    if (p === 'on' || p === 'ontario') return RESSOURCES_ON_FR;
    if (p === 'ab' || p === 'alberta') return RESSOURCES_AB_FR;
    return RESSOURCES_QC_FR;
  } else {
    if (p === 'qc' || p === 'québec' || p === 'quebec') return RESSOURCES_QC_EN;
    if (p === 'on' || p === 'ontario') return RESSOURCES_ON_EN;
    if (p === 'ab' || p === 'alberta') return RESSOURCES_AB_EN;
    return RESSOURCES_QC_EN;
  }
}

// ============================================================
// GÉNÉRATION SECTION 2.2 — RESSOURCES INTERNES
// ============================================================

function buildSection2_2(ctx: DocumentContext, lang: 'fr' | 'en'): PhoneEntry[] {
...
// ============================================================
// GÉNÉRATION SECTION 2.3 — ÉQUIPEMENTS TECHNIQUES
// ============================================================

function buildSection2_3(ctx: DocumentContext, lang: 'fr' | 'en'): PhoneEntry[] {
  const base = lang === 'fr' ? [...EQUIPEMENTS_BASE_FR] : [...EQUIPEMENTS_BASE_EN];
  const cond = lang === 'fr' ? EQUIPEMENTS_CONDITIONNELS_FR : EQUIPEMENTS_CONDITIONNELS_EN;
  const extra: string[] = [];

  if (ctx.has_sprinklers)         extra.push(...cond.has_sprinklers);
  if (ctx.has_generator)          extra.push(...cond.has_generator);
  if (ctx.has_elevators)          extra.push(...cond.has_elevators);
  if (ctx.has_hazardous_materials) extra.push(...cond.has_hazardous_materials);
  ...
  // On insère les extras avant la ligne GardaWorld
...
// ============================================================
// GÉNÉRATION COMPLÈTE MODULE 2 — FR
// ============================================================

function generateModule2FR(ctx: DocumentContext): any {
  const isBureau = ctx.buildingType === 'office' || ctx.buildingType === 'bureau'
    || ctx.buildingType === 'commercial';
    ...
    sections: [
      {
        ...
        editable: true,        // L'utilisateur peut modifier les cellules
      ...
      },
      {
        ...
        allowAdd: true,
        allowDelete: true,
        useDropdown: true,     // Dropdown pour le champ Rôle
        availableRoles: isBureau ? ROLES_INTERNES_BUREAU_FR : ROLES_INTERNES_INDUSTRIEL_FR,
        entries: buildSection2_2(ctx, 'fr'),
      },
      {
        id: '2.3',
        title: 'CONTRACTEURS TECHNIQUES DU BÂTIMENT',
        type: 'phone_table',
        columns: ['RÔLE / ÉQUIPEMENT', 'NOM', 'TÉLÉPHONE'],
        editable: true,
        allowAdd: true,
        allowDelete: true,
        useDropdown: true,     // Dropdown pour le champ Rôle
        availableRoles: ALL_EQUIPEMENTS_FR,
...
// ============================================================
// GÉNÉRATION COMPLÈTE MODULE 2 — EN
// ============================================================

function generateModule2EN(ctx: DocumentContext): any {
  const isBureau = ctx.buildingType === 'office' || ctx.buildingType === 'bureau'
    || ctx.buildingType === 'commercial';
  ...
  return {
    ...
    sections: [
      ...
      },
      {
        ...
        allowDelete: true,
        useDropdown: true,
        availableRoles: isBureau ? ROLES_INTERNES_BUREAU_EN : ROLES_INTERNES_INDUSTRIEL_EN,
        entries: buildSection2_2(ctx, 'en'),
      },
      {
        id: '2.3',
        title: 'BUILDING TECHNICAL CONTRACTORS',
        type: 'phone_table',
        columns: ['ROLE / EQUIPMENT', 'NAME', 'PHONE'],
        editable: true,
        allowAdd: true,
        allowDelete: true,
        useDropdown: true,
        availableRoles: ALL_EQUIPEMENTS_EN,
...
// ============================================================
// EXPORT PRINCIPAL — même signature que generateModule1
// ============================================================

export function generateModule2(ctx: DocumentContext): any {
  return {
