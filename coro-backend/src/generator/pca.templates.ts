...
export function generatePcaModules(ctx: DocumentContext, pcaConfig: any) {
  ...
  const m4fr = {
    ...
    sections: [
      ...
      {
        id: 'm4_s4',
        title: 'Fiches de ressources critiques par activité',
        content: criticalServices.length > 0
          ? criticalServices.map((s: any, i: number) => `FICHE RESSOURCE — ${(s.name || `Service ${i + 1}`).toUpperCase()}\n${'─'.repeat(50)}\n\nObjectif de reprise :\n• RTO : ${s.rto || 'Non défini'} | RPO : ${s.rpo || 'Non défini'} | MAD : ${s.mad || 'Non défini'}\n\nPersonnel minimum requis :\n• Nombre minimum d'employés : À documenter\n• Compétences essentielles : À documenter\n• Personnes de remplacement identifiées : À documenter\n\nSystèmes TI critiques :\n• Applications essentielles : À documenter\n• Accès minimum requis : À documenter\n• Procédure de reprise TI : Voir PC022\n\nÉquipements essentiels :\n• Équipements requis : À documenter\n• Fournisseur de remplacement : À documenter\n• Délai d'approvisionnement : À documenter\n\nFournisseurs critiques :\n• Nom du fournisseur : À documenter\n• Contact d'urgence : À documenter\n• Fournisseur alternatif : ${cfg.alternativeSuppliers ? 'Identifié — voir Section 5' : 'À identifier'}\n\nLocaux requis :\n• Surface minimum : À documenter\n• Équipements de bureau : À documenter\n• Site alternatif : ${cfg.alternativeSiteAddress || 'Voir Section 5'}`
          ).join('\n\n' + '═'.repeat(50) + '\n\n')
          : 'Les fiches de ressources critiques seront générées une fois les services critiques définis dans le BIA (Section 4 du configurateur).',
      },
      {
        id: 'm4_s5',
        title: 'Tolérance à l\'interruption — Tableau récapitulatif',
        content: criticalServices.length > 0
          ? `Tableau de priorisation des activités critiques :\n\n${'─'.repeat(70)}\nActivité critique | RTO | RPO | MAD | Impact financier | Priorité\n${'─'.repeat(70)}\n${criticalServices
              .sort((a: any, b: any) => {
                const order: Record<string, number> = { '1h': 1, '4h': 2, '8h': 3, '24h': 4, '48h': 5, '72h': 6, '1sem': 7, 'plus': 8 };
                return (order[a.rto] || 9) - (order[b.rto] || 9);
              })
              .map((s: any, i: number) => `${(s.name || `Service ${i + 1}`).substring(0, 30).padEnd(30)} | ${(s.rto || 'N/D').padEnd(5)} | ${(s.rpo || 'N/D').padEnd(5)} | ${(s.mad || 'N/D').padEnd(6)} | ${(s.financialImpact || 'N/D').padEnd(16)} | P${i + 1}`)
              .join('\n')}\n${'─'.repeat(70)}\n\nNote : P1 = priorité de reprise la plus élevée (RTO le plus court)`
          : 'Le tableau récapitulatif sera généré une fois le BIA complété.',
      },
    ],
  };
  ...
  const m6fr = {
    ...
    sections: [
      ...
      },
      {
        ...
        content: `CONTACTS INTERNES — CELLULE DE GESTION D'INCIDENT\n${'─'.repeat(60)}\nCooronnateur PCA : ${cfg.coordinatorFirstName || ''} ${cfg.coordinatorLastName || ''}\n  Tél. : ${cfg.coordinatorPhone || 'À compléter'} | Courriel : ${cfg.coordinatorEmail || 'À compléter'}\n\nSubstitut : ${cfg.substituteFirstName || ''} ${cfg.substituteLastName || ''}\n  Tél. : ${cfg.substitutePhone || 'À compléter'} | Courriel : ${cfg.substituteEmail || 'À compléter'}\n\n${(cfg.cellMembers || []).length > 0
          ? (cfg.cellMembers || []).map((m: any) => `${m.role || 'Membre'} : ${m.firstName || ''} ${m.lastName || ''}\n  Tél. : ${m.phone || 'À compléter'} | Courriel : ${m.email || 'À compléter'}`).join('\n\n')
          : 'Membres de la cellule : À documenter'}\n\nPorte-parole : ${cfg.spokesperson || 'À désigner'}\n\n${'═'.repeat(60)}\n\nCONTACTS EXTERNES — SERVICES D'URGENCE\n${'─'.repeat(60)}\n• Services d'urgence (911) : 9-1-1\n• Police locale : À compléter\n• Pompiers (non urgence) : À compléter\n• Ambulance (non urgence) : À compléter\n• Hôpital le plus proche : À compléter\n\n${'═'.repeat(60)}\n\nCONTACTS EXTERNES — SERVICES MUNICIPAUX ET PUBLICS\n${'─'.repeat(60)}\n• Ville / Municipalité : À compléter\n• Hydro-Québec (pannes) : 1 800 790-2424\n• Gaz Métro / Énergir : 1 800 361-8003\n• Télécommunications principal : À compléter\n• Service des eaux : À compléter\n\n${'═'.repeat(60)}\n\nCONTACTS EXTERNES — ASSURANCES\n${'─'.repeat(60)}\n• Courtier d'assurance : À compléter\n• Compagnie d'assurance : À compléter\n• Numéro de police : À compléter\n• Ligne sinistres 24h/7j : À compléter\n• Dernier examen couverture : ${cfg.insuranceLastReview ? new Date(cfg.insuranceLastReview).toLocaleDateString('fr-CA') : 'À compléter'}\n\n${'═'.repeat(60)}\n\nCONTACTS EXTERNES — FOURNISSEURS CRITIQUES\n${'─'.repeat(60)}\n• Fournisseur TI principal : À compléter\n• Fournisseur télécom : À compléter\n• Fournisseur matières premières : À compléter\n• Agence de personnel temporaire : À compléter\n• Entreprise de déménagement d'urgence : À compléter\n\n${'═'.repeat(60)}\n\nCONTACTS EXTERNES — AUTORITÉS RÉGLEMENTAIRES\n${'─'.repeat(60)}\n${(cfg.authoritiesToNotify || []).length > 0
          ? cfg.authoritiesToNotify.map((a: string) => `• ${a} : À compléter`).join('\n')
          : '• Aucune autorité identifiée — À documenter selon les scénarios applicables'}\n• CNESST (accidents travail) : 1 844 838-0808\n• Commission d'accès à l'information (incidents données) : 1 888 528-7741`,
      },
      {
        id: 'm6_s6',
        title: 'Schéma d\'alerte et de mobilisation',
        content: `Le schéma d'alerte définit le processus de notification lors de l'activation du PCA :\n\nÉTAPE 1 — DÉTECTION ET ÉVALUATION INITIALE\n• Témoin de l'incident ou premier gestionnaire évalue la situation\n• Détermine le niveau d'incident (Niveau 1 / 2 / 3 — voir Module 3)\n• Si Niveau 2 ou 3 : contacte immédiatement le Coordonnateur PCA\n\nÉTAPE 2 — NOTIFICATION DU COORDONNATEUR PCA\n• Coordonnateur PCA : ${cfg.coordinatorFirstName || ''} ${cfg.coordinatorLastName || ''} — ${cfg.coordinatorPhone || 'Tél. à compléter'}\n• Si injoignable : Substitut — ${cfg.substituteFirstName || ''} ${cfg.substituteLastName || ''} — ${cfg.substitutePhone || 'Tél. à compléter'}\n\nÉTAPE 3 — ACTIVATION DE LA CELLULE DE GESTION D'INCIDENT\n• Coordonnateur convoque les membres requis selon le niveau d'incident\n• Lieu de coordination : ${cfg.coordinationLocation || 'À définir'}\n• Pont téléphonique : ${cfg.emergencyBridge || 'À définir'}\n• Système d'alerte de masse : ${cfg.massAlertSystem ? 'Disponible — activer' : 'Non disponible — notifications manuelles'}\n\nÉTAPE 4 — COMMUNICATION EXTERNE\n• Canal externe : ${cfg.externalChannel || 'À définir'}\n• Porte-parole : ${cfg.spokesperson || 'À désigner'}\n• Message approuvé : ${cfg.mediaContact ? 'Voir Section 6.4' : 'À préparer'}\n\nÉTAPE 5 — DOCUMENTATION\n• Activer le journal de bord d'incident (voir Module 8)\n• Documenter toutes les décisions et actions prises\n• Heure d'activation du PCA : _______________`,
      },
    ],
  };
  ...
  const m7fr = {
    ...
    sections: [
      ...
      {
        id: 'm7_s5',
        title: 'Liste de vérification d\'activation du PCA',
        content: `La liste de vérification suivante doit être complétée lors de chaque activation du PCA.\nConserver une copie papier accessible en tout temps.\n\n${'═'.repeat(60)}\nLISTE DE VÉRIFICATION — ACTIVATION DU PCA\nDate d'activation : _______________  Heure : _______________\nType d'incident : _______________________________________________\nNiveau d'incident : ☐ Niveau 1  ☐ Niveau 2  ☐ Niveau 3\nActivé par : ___________________________________________________\n${'═'.repeat(60)}\n\nPHASE 1 — ÉVALUATION IMMÉDIATE (0-30 minutes)\n☐ Incident détecté et évalué\n☐ Niveau d'incident déterminé\n☐ Coordonnateur PCA contacté\n☐ Sécurité des personnes assurée\n☐ Autorités d'urgence contactées si requis (9-1-1)\n\nPHASE 2 — MOBILISATION (30-60 minutes)\n☐ Cellule de gestion d'incident convoquée\n☐ Lieu de coordination établi : ${cfg.coordinationLocation || '_______________'}\n☐ Pont téléphonique activé : ${cfg.emergencyBridge || '_______________'}\n☐ Journal de bord d'incident ouvert (Module 8)\n☐ Membres clés de la cellule joints :\n   ☐ ${cfg.coordinatorFirstName || '___'} ${cfg.coordinatorLastName || '___'} (Coordonnateur)\n   ☐ ${cfg.substituteFirstName || '___'} ${cfg.substituteLastName || '___'} (Substitut)\n${(cfg.cellMembers || []).map((m: any) => `   ☐ ${m.firstName || '___'} ${m.lastName || '___'} (${m.role || 'Membre'})`).join('\n')}\n\nPHASE 3 — ACTIVATION DES STRATÉGIES (1-4 heures)\n☐ Stratégies de continuité activées selon le scénario\n☐ Activités critiques priorisées selon le BIA\n${criticalServices.length > 0 ? criticalServices.slice(0, 5).map((s: any, i: number) => `   ☐ ${s.name || `Service ${i + 1}`} (RTO : ${s.rto || 'N/D'})`).join('\n') : '   ☐ Services critiques à documenter'}\n☐ Site alternatif activé si requis : ${cfg.alternativeSiteAddress || 'N/A'}\n☐ Télétravail activé si applicable : ${cfg.teleworkPossible || 'N/A'}\n☐ Fournisseurs alternatifs contactés si requis\n\nPHASE 4 — COMMUNICATION\n☐ Communication interne envoyée (${cfg.internalChannel || 'canal à définir'})\n☐ Clients prioritaires avisés\n☐ Autorités réglementaires avisées si requis\n☐ Communication externe via porte-parole : ${cfg.spokesperson || 'À désigner'}\n\nPHASE 5 — SUIVI ET DOCUMENTATION\n☐ Journal de bord mis à jour en continu\n☐ Rapports de situation envoyés (fréquence : toutes les 2-4 heures)\n☐ Heure de résolution prévue communiquée : _______________\n\nRETOUR À LA NORMALE\n☐ Critères de retour à la normale atteints\n☐ Activités critiques reprises normalement\n☐ Journal de bord fermé — Heure : _______________\n☐ Rapport post-incident initié (dans les 72 heures)\n☐ Leçons apprises documentées`,
      },
      {
        id: 'm7_s6',
        title: 'Procédures de continuité par scénario',
        content: `Les procédures détaillées par scénario sont disponibles dans la bibliothèque de procédures PCA (Module 4) et incluent :\n\n• PC001 — Activation du Plan de continuité des activités\n• PC002 — Déclenchement de la cellule de gestion d'incident\n• PC011 — Sinistre bâtiment — Procédure de continuité\n• PC012 — Cyberincident — Procédure de continuité\n• PC013 — Pandémie / absentéisme massif\n• PC014 — Panne électrique prolongée\n• PC015 — Perte d'un fournisseur critique\n• PC016 — Perte d'un employé clé\n• PC021 — Retour aux activités normales\n\nChaque procédure suit le format standard CORO :\n• Objectif\n• Déclencheur\n• Rôles impliqués\n• Étapes séquentielles\n• Points de vérification\n• Ressources requises\n\n${cfg.linkedPmuId ? 'Note : Ce PCA est lié à un PMU/PSI existant. Les procédures d\'évacuation et d\'intervention immédiate sont couvertes par ce plan complémentaire.' : 'Note : Il est recommandé de créer un PMU ou PSI complémentaire pour les procédures d\'évacuation et d\'intervention immédiate.'}`,
      },
    ],
  };
  ...
  const m8fr = {
    ...
    sections: [
      ...
      },
      {
        ...
        content: `Le journal de bord doit être ouvert dès l'activation du PCA et maintenu en continu jusqu'à la résolution de l'incident.\nConserver des copies papier accessibles en tout temps.\n\n${'═'.repeat(60)}\nJOURNAL DE BORD D'INCIDENT\n${'═'.repeat(60)}\nDate d'ouverture : _______________  Heure : _______________\nType d'incident : _____________________________________________\nNiveau : ☐ Niveau 1  ☐ Niveau 2  ☐ Niveau 3\nResponsable du journal : _____________________________________\n${'─'.repeat(60)}\n\nCHRONOLOGIE DES ACTIONS\n${'─'.repeat(60)}\nHeure | Action / Décision | Responsable | Résultat\n${'─'.repeat(60)}\n___:___ | _________________________________ | _____________ | _____________\n___:___ | _________________________________ | _____________ | _____________\n___:___ | _________________________________ | _____________ | _____________\n___:___ | _________________________________ | _____________ | _____________\n___:___ | _________________________________ | _____________ | _____________\n___:___ | _________________________________ | _____________ | _____________\n___:___ | _________________________________ | _____________ | _____________\n___:___ | _________________________________ | _____________ | _____________\n${'─'.repeat(60)}\n\nSTATUT DES ACTIVITÉS CRITIQUES\n${'─'.repeat(60)}\n${criticalServices.length > 0
          ? criticalServices.map((s: any) => `${(s.name || 'Activité').substring(0, 35).padEnd(35)} | ☐ Normal  ☐ Dégradé  ☐ Suspendu`).join('\n')
          : 'Activité 1 : _________________________ | ☐ Normal  ☐ Dégradé  ☐ Suspendu\nActivité 2 : _________________________ | ☐ Normal  ☐ Dégradé  ☐ Suspendu\nActivité 3 : _________________________ | ☐ Normal  ☐ Dégradé  ☐ Suspendu'}\n${'─'.repeat(60)}\n\nRAPPORTS DE SITUATION (toutes les 2-4 heures)\n${'─'.repeat(60)}\nRapport 1 — Heure : _______  Envoyé à : _____________________\nContenu : ___________________________________________________\n\nRapport 2 — Heure : _______  Envoyé à : _____________________\nContenu : ___________________________________________________\n\nRapport 3 — Heure : _______  Envoyé à : _____________________\nContenu : ___________________________________________________\n${'─'.repeat(60)}\n\nFERMETURE DU JOURNAL\nDate de fermeture : _______________  Heure : _______________\nCritères de retour à la normale atteints : ☐ Oui  ☐ Non\nSignature du coordonnateur PCA : _____________________________`,
      },
      {
        id: 'm8_s7',
        title: 'Rapport post-incident',
        content: `Le rapport post-incident doit être complété dans les 72 heures suivant la résolution de l'incident.\nIl sert à documenter les leçons apprises et améliorer le PCA.\n\n${'═'.repeat(60)}\nRAPPORT POST-INCIDENT\n${'═'.repeat(60)}\nDate de l'incident : _______________\nDate de résolution : _______________\nDurée totale de l'incident : _______________\nNiveau d'incident : ☐ Niveau 1  ☐ Niveau 2  ☐ Niveau 3\nType d'incident : _____________________________________________\nRédigé par : _________________________  Date : _______________\n${'─'.repeat(60)}\n\n1. RÉSUMÉ DE L'INCIDENT\n_______________________________________________________________\n_______________________________________________________________\n_______________________________________________________________\n\n2. CHRONOLOGIE RÉSUMÉE\nDébut : _______________\nActivation PCA : _______________\nRésolution : _______________\nÉvénements clés :\n• _______________________________________________________________\n• _______________________________________________________________\n• _______________________________________________________________\n\n3. IMPACT SUR LES ACTIVITÉS\n${'─'.repeat(60)}\n${criticalServices.length > 0
          ? criticalServices.map((s: any) => `${s.name || 'Activité'} :\n  Impact : ☐ Aucun  ☐ Mineur  ☐ Majeur  ☐ Critique\n  Durée interruption : _______________\n  RTO respecté : ☐ Oui  ☐ Non`).join('\n\n')
          : 'Activité 1 :\n  Impact : ☐ Aucun  ☐ Mineur  ☐ Majeur  ☐ Critique\n  Durée interruption : _______________\n  RTO respecté : ☐ Oui  ☐ Non'}\n\n4. STRATÉGIES ACTIVÉES\n☐ Télétravail\n☐ Site alternatif\n☐ Fournisseur alternatif\n☐ Personnel temporaire\n☐ Procédures manuelles\n☐ Autre : _______________________________________________\n\n5. CE QUI A BIEN FONCTIONNÉ\n• _______________________________________________________________\n• _______________________________________________________________\n• _______________________________________________________________\n\n6. CE QUI DOIT ÊTRE AMÉLIORÉ\n• _______________________________________________________________\n• _______________________________________________________________\n• _______________________________________________________________\n\n7. ACTIONS CORRECTIVES\n${'─'.repeat(60)}\nAction | Responsable | Échéance | Statut\n${'─'.repeat(60)}\n_________________________________ | _____________ | __________ | ________\n_________________________________ | _____________ | __________ | ________\n_________________________________ | _____________ | __________ | ________\n\n8. MISE À JOUR DU PCA REQUISE\n☐ Oui — Sections à mettre à jour : ___________________________\n☐ Non\n\nSignature du coordonnateur PCA : _____________________________\nSignature de la direction : __________________________________`,
      },
      {
        id: 'm8_s8',
        title: 'Formulaire d\'évaluation des exercices',
        content: `À compléter après chaque exercice de continuité.\n\n${'═'.repeat(60)}\nFORMULAIRE D'ÉVALUATION D'EXERCICE\n${'═'.repeat(60)}\nDate de l'exercice : _______________\nType d'exercice : ☐ Formatif/Discussion  ☐ Sur table  ☐ Simulation  ☐ Test TI\nScénario utilisé : _____________________________________________\nAnimateur : ___________________________________________________\nNombre de participants : _______\n${'─'.repeat(60)}\n\nPARTICIPANTS\n${'─'.repeat(60)}\nNom | Rôle | Présent\n${'─'.repeat(60)}\n${(cfg.cellMembers || []).length > 0
          ? (cfg.cellMembers || []).map((m: any) => `${(m.firstName || '') + ' ' + (m.lastName || '')} | ${m.role || ''} | ☐`).join('\n')
          : '_________________________ | _________________ | ☐\n_________________________ | _________________ | ☐\n_________________________ | _________________ | ☐'}\n${'─'.repeat(60)}\n\nÉVALUATION DES OBJECTIFS\n${'─'.repeat(60)}\nObjectif | Atteint | Commentaires\n${'─'.repeat(60)}\nCompréhension des rôles | ☐ Oui  ☐ Partiel  ☐ Non | ________________\nCommunication efficace | ☐ Oui  ☐ Partiel  ☐ Non | ________________\nActivation procédures | ☐ Oui  ☐ Partiel  ☐ Non | ________________\nRTO respectés | ☐ Oui  ☐ Partiel  ☐ Non | ________________\nOutils utilisés correctement | ☐ Oui  ☐ Partiel  ☐ Non | ________________\n${'─'.repeat(60)}\n\nLACUNES IDENTIFIÉES\n• _______________________________________________________________\n• _______________________________________________________________\n• _______________________________________________________________\n\nAMÉLIORATIONS RECOMMANDÉES\n• _______________________________________________________________\n• _______________________________________________________________\n• _______________________________________________________________\n\nNOTE GLOBALE DE L'EXERCICE\n☐ Excellent  ☐ Bon  ☐ À améliorer  ☐ Insuffisant\n\nProchaine date d'exercice recommandée : _______________\nSignature de l'animateur : __________________________________`,
      },
    ],
  };
