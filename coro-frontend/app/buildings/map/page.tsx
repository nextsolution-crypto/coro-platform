...
import { FileText, X, Maximize2, Minimize2, Users } from 'lucide-react';
...
function getBuildingStatus(building: any): { color: string; glow: string; label: string } {
  const projects = building.projects || [];
  if (projects.length === 0) return { color: '#4A5568', glow: 'rgba(74,85,104,0.4)', label: 'Aucun document' };
  ...
  if (hasExpired) return { color: '#E74C3C', glow: 'rgba(231,76,60,0.5)', label: 'À renouveler' };
  const hasInProgress = projects.some((p: any) => ['DRAFT', 'IN_PROGRESS', 'REVIEW'].includes(p.status));
  if (hasInProgress) return { color: '#F39C12', glow: 'rgba(243,156,18,0.5)', label: 'En cours' };
  const allValidated = projects.every((p: any) => p.status === 'VALIDATED');
  if (allValidated) return { color: '#27AE60', glow: 'rgba(39,174,96,0.5)', label: 'À jour' };
  return { color: '#7F8C8D', glow: 'rgba(127,140,141,0.4)', label: 'Partiel' };
}
...
export default function BuildingsMapPage() {
  ...
  const clients = Array.from(
    new Map(buildings.map(b => [b.client.id, b.client])).values()
  );

  const filteredBuildings = clientFilter
    ? buildings.filter(b => b.client.id === clientFilter)
    : buildings;
  ...
  useEffect(() => {
    ...
    initMap(filteredBuildings);
  }, [loading, filteredBuildings]);

  const initMap = (bldgs: any[]) => {
    ...
    const withCoords = bldgs.filter(b => b.latitude && b.longitude);
    ...
    styleEl.id = 'coro-map-dark';
    if (!document.getElementById('coro-map-dark')) {
      styleEl.textContent = `.leaflet-tile { filter: invert(1) hue-rotate(180deg) brightness(0.85) saturate(0.7) contrast(0.9) !important; }`;
    ...
    withCoords.forEach(building => {
      ...
      const markerHtml = `
        <div style="position:relative;width:36px;height:36px;display:flex;align-items:center;justify-content:center;">
          <div style="position:absolute;width:36px;height:36px;border-radius:50%;background:${status.color};opacity:0.2;animation:pulse 2s ease-out infinite;"></div>
          <div style="position:absolute;width:24px;height:24px;border-radius:50%;background:${status.color};opacity:0.4;animation:pulse 2s ease-out infinite;animation-delay:0.3s;"></div>
          <div style="position:relative;width:14px;height:14px;border-radius:50%;background:${status.color};border:2px solid rgba(255,255,255,0.8);box-shadow:0 0 12px ${status.glow},0 0 24px ${status.glow};z-index:10;"></div>
        </div>`;
      ...
      marker.bindTooltip(`
        <div style="font-size:12px;font-weight:700;color:#E2E8F0;">${building.name}</div>
        <div style="font-size:10px;color:#8899AA;margin-top:2px;">${building.client?.name || ''}</div>
      `, {
        permanent: false, direction: 'top', offset: [0, -22], className: 'coro-dark-tooltip',
      });
  ...
  const stats = {
    ...
    aJour: filteredBuildings.filter(b => getBuildingStatus(b).label === 'À jour').length,
    enCours: filteredBuildings.filter(b => getBuildingStatus(b).label === 'En cours').length,
    aRenouveler: filteredBuildings.filter(b => getBuildingStatus(b).label === 'À renouveler').length,
    sansDoc: filteredBuildings.filter(b => getBuildingStatus(b).label === 'Aucun document').length,
  };

  const withCoords = filteredBuildings.filter(b => b.latitude && b.longitude);
  ...
  const mapContent = (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#FFFFFF' }}>
            Vue cartographique des bâtiments
          </p>
        </div>
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: selectedStatus.color, boxShadow: `0 0 8px ${selectedStatus.glow}`, flexShrink: 0 }} />
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
            {selected.projects && selected.projects.length > 0 ? (
              <>
                <p style={{ margin: '0 0 12px', fontSize: 10, fontWeight: 700, color: '#4A5568', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Documents</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {selected.projects.map((p: any) => {
                    ...
                    return (
                      ...
                      >
                        <div style={{ width: 34, height: 34, borderRadius: 6, backgroundColor: `${dc}22`, border: `1px solid ${dc}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <span style={{ fontSize: 10, fontWeight: 900, color: dc }}>{p.documentType}</span>
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ margin: '0 0 2px', fontSize: 13, fontWeight: 600, color: '#E2E8F0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</p>
                          <p style={{ margin: 0, fontSize: 11, color: '#4A5568' }}>{p.year}</p>
                        </div>
                        <span style={{ fontSize: 10, fontWeight: 600, padding: '3px 7px', borderRadius: 6, backgroundColor: sc.bg, color: sc.text, border: `1px solid ${sc.border}`, whiteSpace: 'nowrap', flexShrink: 0 }}>
                          {sc.label}
                        </span>
                      </button>
                    );
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 20 }}>
              <button type="button" onClick={() => router.push(`/buildings/${selected.id}`)}
                style={{ padding: '10px 12px', borderRadius: 7, fontSize: 12, fontWeight: 600, backgroundColor: 'rgba(41,128,185,0.15)', color: '#5DADE2', border: '1px solid rgba(41,128,185,0.3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
              ...
              >
                <FileText size={13} /> Fiche bâtiment
              </button>
              <button type="button" onClick={() => router.push(`/projects?building=${selected.id}`)}
                style={{ padding: '10px 12px', borderRadius: 7, fontSize: 12, fontWeight: 600, backgroundColor: 'rgba(39,174,96,0.15)', color: '#58D68D', border: '1px solid rgba(39,174,96,0.3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
                onMouseEnter={e => e.currentTarget.style.backgroundColor = 'rgba(39,174,96,0.25)'}
                onMouseLeave={e => e.currentTarget.style.backgroundColor = 'rgba(39,174,96,0.15)'}
              >
                <Users size={13} /> Projets
              </button>
            </div>
          </div>
        </div>
      ...
      {!selected && filteredBuildings.length > 0 && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 10, overflowX: 'auto', paddingBottom: 4, pointerEvents: 'all' }}>
            {filteredBuildings.map(b => {
              ...
              return (
                <button key={b.id} type="button"
                  ...
                  style={{
                    flexShrink: 0, width: 170, padding: '10px 12px',
                    backgroundColor: 'rgba(13,20,30,0.9)',
                    border: `1px solid ${st.color}44`, borderRadius: 8,
                    cursor: 'pointer', textAlign: 'left', backdropFilter: 'blur(12px)',
                  }}
                  onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.borderColor = st.color; }}
                  onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'rgba(13,20,30,0.9)'; e.currentTarget.style.borderColor = `${st.color}44`; }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    <div style={{ width: 7, height: 7, borderRadius: '50%', backgroundColor: st.color, boxShadow: `0 0 5px ${st.glow}`, flexShrink: 0 }} />
                    <span style={{ fontSize: 9, color: st.color, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{st.label}</span>
                  </div>
                  <p style={{ margin: '0 0 2px', fontSize: 12, fontWeight: 700, color: '#E2E8F0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.name}</p>
                  <p style={{ margin: '0 0 6px', fontSize: 10, color: '#4A5568', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.client?.name}</p>
          ...
          {clients.length > 1 && (
            <select
              ...
              style={{
                ...
                border: '1px solid #DEE2E6', color: '#2C3E50',
                backgroundColor: '#FFFFFF', cursor: 'pointer',
              }}
            >
              <option value="">Tous les clients ({buildings.length})</option>
              {clients.map(c => (
                <option key={c.id} value={c.id}>
                  {c.name} ({buildings.filter(b => b.client.id === c.id).length})
                </option>
              ))}
            </select>
          )}
        </div>
        {withCoords.length < filteredBuildings.length && (
          <div style={{ marginTop: 10, padding: '8px 12px', borderRadius: 7, backgroundColor: '#FEF9E7', border: '1px solid #FAD7A0', fontSize: 12, color: '#F39C12' }}>
            ⚠️ {filteredBuildings.length - withCoords.length} bâtiment{filteredBuildings.length - withCoords.length > 1 ? 's' : ''} sans coordonnées GPS — modifiez-les pour les afficher.
          </div>
        )}
      </div>
      {mapContent}
