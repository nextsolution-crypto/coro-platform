...
export default function TaskTemplatesPage() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await api.post('/task-templates', newTask);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Désactiver ce template de tâche ?')) return;
    try {
        ...
        <div className="rounded-md p-4 sm:p-5 mb-6" style={{ backgroundColor: '#FFFFFF', border: '1px solid #AED6F1' }}>
          ...
          <div className="mb-4">
            ...
            <input type="text" value={newTask.taskTitle}
              ...
              placeholder="Ex: Visite pour le relevé technique"
              className="w-full px-3 py-2 text-sm rounded"
              style={{ border: '1px solid #CED4DA', color: '#2C3E50' }} />
          </div>
          <div className="mb-4">
            ...
            <div className="flex gap-2 flex-wrap">
              {DOCUMENT_TYPES.map(type => (
                <button key={type} onClick={() => setNewTask({ ...newTask, documentTypes: toggleDocType(newTask.documentTypes, type) })}
