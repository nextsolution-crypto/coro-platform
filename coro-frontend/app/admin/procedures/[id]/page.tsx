...
function StepEditor({
  ...
  const addSubStep = () => {
    onUpdate({
      ...
      subSteps: [...(step.subSteps || []), {
        id: generateId(), textFR: '', textEN: '', isList: true,
      }],
    });
  };
  ...
  const deleteSubStep = (idx: number) => {
    const subSteps = (step.subSteps || []).filter((_, i) => i !== idx);
    onUpdate({ ...step, subSteps });
  };
      ...
      <div className="p-3">
        <div className="flex items-start gap-2 mb-2">
          <button onClick={() => setExpanded(!expanded)} className="flex-shrink-0 mt-1">
            {expanded ? <ChevronUp size={14} style={{ color: '#ADB5BD' }} /> : <ChevronDown size={14} style={{ color: '#ADB5BD' }} />}
          </button>
...
function RoleSectionEditor({
  ...
  const addStep = () => {
    onUpdate({
      ...section,
      steps: [...section.steps, { id: generateId(), textFR: '', textEN: '' }],
    });
  };
...
export default function ProcedureEditPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && user?.role === 'SUPER_ADMIN' && id) fetchData();
    else if (isAuthenticated && user?.role !== 'SUPER_ADMIN') router.push('/dashboard');
  }, [isAuthenticated, user, id]);

  const fetchData = async () => {
    try {
      const res = await api.get(`/procedures/${id}`, { params: { organizationId: 'default' } });
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      await api.put(`/procedures/${id}/default`, { content });
      setSaved(true);
  ...
  const addDirective = () => {
    ...
    setContent({
      ...
      directivesGenerales: [...(content.directivesGenerales || []), {
        id: generateId(), textFR: '', textEN: '', isBold: false,
      }],
    });
  };
  ...
  if (loading) {
    return <AppLayout><div className="text-center py-12"><p className="text-sm animate-pulse" style={{ color: '#ADB5BD' }}>Chargement...</p></div></AppLayout>;
  }

  if (!content) {
    return <AppLayout><div className="text-center py-12"><p className="text-sm" style={{ color: '#C0392B' }}>Procédure introuvable.</p></div></AppLayout>;
  }
    ...
    <AppLayout>
      ...
      <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4 mb-8">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <h2 className="text-xl font-semibold mt-1 break-words" style={{ color: '#2C3E50' }}>{content.titleFR}</h2>
            <p className="text-sm break-words" style={{ color: '#ADB5BD' }}>{content.titleEN}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 w-full xl:w-auto">
          ...
          <button
            ...
            className="w-full sm:w-auto justify-center flex items-center gap-2 px-3 py-2 rounded text-sm transition-colors"
      ...
      <div className="rounded-md p-4 sm:p-6 mb-6" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E9ECEF' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-semibold mb-1 uppercase" style={{ color: '#6C757D' }}>Couleur (hex)</label>
            <div className="flex items-center gap-2">
          ...
          <div>
            <label className="block text-xs font-semibold mb-1 uppercase" style={{ color: '#6C757D' }}>Icône (emoji)</label>
      ...
      <div className="rounded-md p-4 sm:p-6 mb-6" style={{ backgroundColor: '#FFFFFF', border: '1px solid #E9ECEF' }}>
        ...
        {(content.directivesGenerales || []).length === 0 && (
          <p className="text-sm" style={{ color: '#ADB5BD' }}>Aucune directive générale pour cette procédure.</p>
        )}
