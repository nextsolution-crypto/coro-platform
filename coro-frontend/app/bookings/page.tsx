                ...
                <div className="p-4 sm:p-5">
                  ...
                  <div className="mt-4 rounded p-3" style={{ backgroundColor: '#F8F9FA', border: '1px solid #E9ECEF' }}>
                    ...
                    {(() => { const candidate = availableByBooking[booking.id]?.find(u => u.id === teamUserIds[booking.id]); return candidate && <div className="text-xs mt-2" aria-live="polite">
                      ...
                      {(candidate.conflicts || []).map((conflict: any) => <p key={`${conflict.source}-${conflict.sourceId}-${conflict.startUtc}`}>{conflict.source === 'USER_UNAVAILABILITY' ? 'Indisponible' : conflict.source === 'WORK_SCHEDULE' ? 'Hors horaire de travail' : conflict.source === 'LEGACY_ACTIVITY' ? 'Conflit potentiel — activité legacy' : conflict.severity === 'BLOCKED' ? 'Engagement confirmé' : 'Proposition provisoire'} ({new Date(conflict.startUtc).toLocaleString('fr-CA', { timeZone: conflict.timeZone })})</p>)}
